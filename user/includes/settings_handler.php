...
if (isset($_POST['update_details'])) {
	...
	if($matched_user == "" || $matched_user == $userLoggedIn) {
		...
		$query = mysqli_query($con, "UPDATE user SET name='$name', phone='$phone', email='$email' WHERE id='$userLoggedIn'");
	}
	else 
	...
	$password_query = mysqli_query($con, "SELECT password FROM user WHERE id='$userLoggedIn'");
	...
	if(md5($old_password) == $db_password) {
		...
		if($new_password_1 == $new_password_2) {
			...
			if(strlen($new_password_1) <= 4) {
				...
				$password_query = mysqli_query($con, "UPDATE user SET password='$new_password_md5' WHERE id='$userLoggedIn'");
